...
export function creatModal() {
	...
	recupererTravail().then(données => afficherImagesModal(données));
...
export function recupererTravail() {
	return fetch('http://localhost:5678/api/works').then(response => response.json()).catch(erreur => console.error(erreur));
}
...
export function creatForm(modalContainer) {
	...
	fetch('http://localhost:5678/api/categories').then((response) => response.json()).then((categories) => {
